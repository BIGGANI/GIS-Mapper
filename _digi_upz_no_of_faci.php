<?php
$divisions_q=mysql_query("SELECT *,COUNT(*) AS cnnt, orga.*, upza.name AS upzname FROM organization orga
JOIN upazillas upza
ON orga.upazilla_id=upza.upazilla_id
WHERE orga.district_id='$page'
GROUP BY orga.upazilla_id
ORDER BY cnnt ASC");
$div_datalist="";
...
while($divisions_qr=mysql_fetch_object($divisions_q)){
 ...
 $all_NoOfFaci_q=mysql_query("SELECT *,COUNT(*) AS cnnt, orga.*, upza.name AS upzname FROM organization orga
JOIN upazillas upza
ON orga.upazilla_id=upza.upazilla_id
WHERE orga.district_id='$page'
GROUP BY orga.upazilla_id
