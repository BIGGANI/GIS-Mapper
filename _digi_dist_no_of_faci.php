<?php
$divisions_q=mysql_query("SELECT *,COUNT(*) AS cnnt, orga.*, dist.name AS distname FROM organization orga
JOIN districts dist
ON orga.district_id=dist.district_id
WHERE orga.division_id='$page'
GROUP BY orga.district_id
ORDER BY cnnt ASC");
$div_datalist="";
...
while($divisions_qr=mysql_fetch_object($divisions_q)){
 ...
 $all_NoOfFaci_q=mysql_query("SELECT *,COUNT(*) AS cnnt, orga.*, dist.name AS distname FROM organization orga
JOIN districts dist
ON orga.district_id=dist.district_id
WHERE orga.division_id='$page'
GROUP BY orga.district_id
ORDER BY cnnt DESC LIMIT 1");
 ...
 $all_NoOfFaci=$all_NoOfFaci_qr->cnnt;
  ...
  $divid="div_".$divisions_qr->district_id;
 ${$divid}=$divisions_qr->district_id;
 
 $distname=$divisions_qr->distname;
...
<div style=\"text-align:center;width:100px; height:30px; background-color:#000000; color:white;\">No Data(0%)</div><br/>";
 ...
 <span style=color:#".${$divColor}.";>".$divisions_qr->distname." District # ".${$div}."</span><br/>".$div_datalist;
