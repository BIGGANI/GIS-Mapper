<?php
$divisions_q=mysql_query("SELECT *,COUNT(*) AS cnnt, orga.*, divi.name AS diviname FROM organization orga
JOIN divisions divi
ON orga.division_id=divi.division_id
GROUP BY orga.division_id
ORDER BY cnnt ASC");
$div_datalist="";
...
while($divisions_qr=mysql_fetch_object($divisions_q)){
 ...
 $all_NoOfFaci_q=mysql_query("SELECT COUNT(*) AS cnnt FROM organization orga
JOIN divisions divi
ON orga.division_id=divi.division_id
GROUP BY orga.division_id
ORDER BY cnnt DESC LIMIT 1");
 ...
 $div=$divisions_qr->diviname."_div_NoOfFaci";
 ${$div}=$divisions_qr->cnnt;
 
  $divid="div_".$divisions_qr->division_id;
 ${$divid}=$divisions_qr->division_id;
 
 $divOpac=$divisions_qr->diviname."_div_Opac";
...
// $bg_color_value=round(${$divOpac}*100, 2);
 $divO=0;
 $divColor=$divisions_qr->diviname."_divColor";
...
<div style=\"text-align:center;width:100px; height:30px; background-color:#000000; color:white;\">No Data(0%)</div><br/>";
 ...
 <span style=color:#".${$divColor}.";>".$divisions_qr->diviname." Division # ".${$div}."</span><br/>".$div_datalist;
